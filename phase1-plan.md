...
- [x] **Task 6** — Minimal Transcription UI (6.1–6.5 done in same pass)
  - `Views/TranscriptionView.swift` — user-facing screen. `@Observable @MainActor TranscriptionModel` owns a weak ref to the pipeline and projects its two streams + `currentRMS` / `framesCaptured` pollers onto `rows`, `isRunning`, `isSpeaking`, `rms`, `elapsed`, `errorMessage`. Subviews: big `ScrollView` with `LazyVStack` of `TranscriptRowView` (partial italic/secondary with trailing "…"; final primary + "raw"/"punctuated" caption; failed red with exclamation), MM:SS elapsed timer, compact mic level meter (`MicLevelMeter`) + speaking indicator dot (`SpeakingIndicator`, green while active), big Record/Stop button that tints red while running. Trash button in toolbar clears the transcript (disabled while running). `partialIntervalMs` defaults to 1500 ms so live growing-text works out of the box.
  - `Pipeline/TranscriptRow.swift` — shared UI projection of `TranscriptUpdate` with `upsert(_:)` helper on `Array<TranscriptRow>`. One implementation, two views (transcription + debug).
  - `Views/ContentView.swift` — refactored: `TranscriptionView` is the primary destination; a hammer-icon toolbar button navigates to a new `DeveloperView` that lists the VAD debug view + smoke-test buttons + "Reinstall Models". Phase 1 users land on the recorder; developers hop into the inspector.
  - `Views/AudioCaptureDebugView.swift` — rewritten as a pipeline consumer: no longer owns audio/VAD/ASR. Subscribes to the three streams (`updates`, `events`, `debugEvents`). New toggles: "Cloud punctuation" (flips `enableCloudPunctuation`) and "Dump utterance WAV" (flips `debugDumpUtterances` to keep smoke tests usable).
  - `TranscriptionModel.friendlyMessage(for:)` maps `AudioCaptureError.permissionDenied` into "Microphone access was denied. Enable it in Settings → VoxLocal…" and similar for the other audio failure cases; model-loading and model-load-failure states are read directly from `AppState.transcriberStatus`.
  - **Deferred**: auto-scroll to top on new utterance (SwiftUI `ScrollViewReader` fiddle — not blocking, newest-first layout means new content is already visible if the user is scrolled up). Copy/export of `TranscriptionSession.fullTranscript` (Phase 2 work). UI indicator for punctuation retry backlog (hook available via `pipeline.pendingPunctuationCount` — not wired yet).
