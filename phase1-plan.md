...
- [x] **Task 4** — Moonshine ASR Integration (complete through subtasks 4.1–4.6; plan.md's 4.3 streaming partials deferred)
  ...
  - **Known Phase 1 limitation (not a bug)**: transcripts only emit after VAD closes an utterance (≥ 700ms of silence per `VADConfiguration.endOfSpeechSilence`). Continuous speech without pauses produces nothing until the `maxUtteranceDuration` cap. Streaming partial results during active speech was Task 4.3 in plan.md and was deliberately deferred to keep Phase 1 scope tight; revisit as part of Phase 2 or as a dedicated follow-up if it hurts UX.
