...
- **5.9** Local FastAPI stub at `tools/punctuation_stub.py` for iOS integration testing ahead of the real server. Returns a visually-distinct punctuated version (first letter uppercased + trailing period). Flip flags (delay, fail status, malformed body) for exercising each iOS fallback path (commit TBD).
